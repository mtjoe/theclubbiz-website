...
$(window).load(function() {
	...
	$('.tab').click(function() {
		...
		if (tab_opened === false){
			...
			$('.event-tabs').animate({top:"100px"}, 500);
			$('.event-tab-pages').find(a).animate({top:"200px"}, 500,
				function(){
					$('.event-tab-pages').find(a).animate({top:"150px"}, 300);
					$('.event-tabs').animate({
						height:"50px"
					}, 300);
			...
			$('.event-tab-pages').find(a).css({"top":"100%"});
			a = "." + $(this).attr('class').split(' ')[1];
			$('.event-tab-pages').find(a).css({"top":"150px"});
		}
	});

	$('.tab-page .exit').click(function() {
		tab_opened = false;
		$('.event-tabs').animate({top:($(window).height() - 99).toString()}, 500);
		$(this).parent().animate({top:($(window).height() - 50).toString()}, 500,
			function() {
				$('.event-tab-pages').find(a).animate({top:($(window).height()+1).toString()}, 300);
				$('.event-tabs').animate({
					height:'100px'
				}, 300);
				$('.tab').children().animate({
